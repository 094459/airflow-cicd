...
class MwaaCdkStackPrdEnv(Stack):
    ...
    def __init__(self, scope: Construct, id: str, vpc, mwaa_props,  **kwargs) -> None:
        ...
        dags_bucket = s3.Bucket(
            self,
            "mwaa-dags",
            bucket_name=f"{mwaa_props['dagss3location'].lower()}-prd",
            versioned=True,
        ...
        mwaa_policy_document = iam.PolicyDocument(
            statements=[
                iam.PolicyStatement(
                    ...
                    resources=[f"arn:aws:airflow:{self.region}:{self.account}:environment/{mwaa_props['mwaa_env']}-prd"],
                ),
                iam.PolicyStatement(
                    actions=[
                        "s3:ListAllMyBuckets"
        ...
        security_group = ec2.SecurityGroup(
            ...
            id = "mwaa-sg-prd",
            vpc = vpc,
            security_group_name = "mwaa-sg-prd"
        )

        security_group_id = security_group.security_group_id

        security_group.connections.allow_internally(ec2.Port.all_traffic(),"MWAA-prd")
        ...
        key = kms.Key(
            self,
            f"{mwaa_props['mwaa_env']}{key_suffix}-prd",
            enable_key_rotation=True,
            policy=kms_mwaa_policy_document
        )

        key.add_alias(f"alias/{mwaa_props['mwaa_env']}{key_suffix}-prd")

        # Create MWAA environment using all the info above

        managed_airflow = mwaa.CfnEnvironment(
            scope=self,
            id='airflow-test-environment-prd',
            name=f"{mwaa_props['mwaa_env']}-prd",
